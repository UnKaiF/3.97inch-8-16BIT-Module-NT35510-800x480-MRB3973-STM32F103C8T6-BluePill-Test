...
void TP_Save_Adjdata(void)
{
	...
	memcpy(eepram, &temp, 4);
	temp=tp_dev.yfac*100000000;//保存y校正因素
    //AT24CXX_WriteLenByte(SAVE_ADDR_BASE+4,temp,4);
	memcpy(eepram+4, &temp, 4);
	//保存x偏移量
    //AT24CXX_WriteLenByte(SAVE_ADDR_BASE+8,tp_dev.xoff,2);
	memcpy(eepram+8, &(tp_dev.xoff), 2);
	//保存y偏移量
	//AT24CXX_WriteLenByte(SAVE_ADDR_BASE+10,tp_dev.yoff,2);
	memcpy(eepram+10, &(tp_dev.yoff), 2);
	//保存触屏类型
	//AT24CXX_WriteOneByte(SAVE_ADDR_BASE+12,tp_dev.touchtype);
	memcpy(eepram+12, &(tp_dev.touchtype), 1);
	temp=0X0A;//标记校准过了
	//AT24CXX_WriteOneByte(SAVE_ADDR_BASE+13,temp);
	memcpy(eepram+13, &temp, 1);
}
...
u8 TP_Get_Adjdata(void)
{
	...
	memcpy(&tmp, eepram+13, 1);
	if(tmp == 0x0A)//触摸屏已经校准过了
	{
		//tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE,4);
		memcpy(&tempfac, eepram, 4);
		tp_dev.xfac=(float)tempfac/100000000;//得到x校准参数
		//tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+4,4);
		memcpy(&tempfac, eepram + 4, 4);
		tp_dev.yfac=(float)tempfac/100000000;//得到y校准参数
	    //得到x偏移量
		//tp_dev.xoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+8,2);
		memcpy(&(tp_dev.xoff), eepram+8, 2);
 	    //得到y偏移量
		//tp_dev.yoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);
		memcpy(&(tp_dev.yoff), eepram+10, 2);
 		//tp_dev.touchtype=AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12);//读取触屏类型标记
		memcpy(&(tp_dev.touchtype), eepram+12, 1);
